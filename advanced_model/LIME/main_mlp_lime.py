...
def train_occ(x_train_, y_train_, x_test_, y_test_, train_sen):
    ...
    mlp_model.lime_exp(rule_combinations_test[0], train_sen)
...
def get_input_combinations(text_ls, rule_or_text):
    ...
    for input_data in text_ls:
        ...
        if rule_or_text == 'rule':
            ...
            rule_comb_2.append(text + '\t' + tense + '\t' + polarity)
            ...
            text_comb_1.append(text + '\t' + osp + '\t' + tense)

    if rule_or_text == 'rule':
        return [rule_comb_1, rule_comb_2]
    elif rule_or_text == 'text':
        return [text_comb_1]
...
x_train = vectorizer.fit_transform(rule_combinations_train[0]).toarray()
...
train_occ(x_train, y_train, x_test, y_test, rule_combinations_train[0])
