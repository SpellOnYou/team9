...
class MultipleInputsModel:
    ...
    def __init__(self, len_of_features, len_of_input, epochs=20, bs=64, lr=0.0001, opt=Adam):
        ...
        self.concat_layer = Concatenate()([self.model_layer, self.dense_layer_1])
        ...
        self.dropout = tf.keras.layers.Dropout(0.5)(self.dense_layer_4)
        self.outputs = tf.keras.layers.Dense(7, activation="softmax", name="output")(self.dropout)
    ...
    def fit_model(self, dnn_model, x1_train, x2_train, y_train):
        ...
        dnn_model.compile(optimizer=optimizer, loss=loss, metrics=[tf.keras.metrics.Recall(),
                                                                   ...
                                                                   tfa.metrics.F1Score(num_classes=7, average='macro')])

        #tfa.metrics.F1Score(num_classes=7)
    ...
    def evaluate(self, dnn_model, test1_x, test2_x, test_y):
        ...
        target_names = ["Guilt", "Shame", "Joy", "Anger", "Fear", "Sadness", "Disgust"]
