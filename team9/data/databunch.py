...
class DataBunch():
    ...
    def get_embedding(self, **kwargs):
        ...
        if self.kwargs['emb_type']=='tfidf':
            ...
            self.x_valid = self.embedder.fit_transform(self.x_valid_text)
            self.x_test = self.embedder.fit_transform(self.x_test_text)

        #when it's pretrained version
        else:
            self.embedder = self._load_pretrained
            self.x_train = np.array([self.embedder[pad_sent, ] for pad_sent in self.train_pad_nums])
            self.x_valid = np.array([self.embedder[pad_sent, ] for pad_sent in self.valid_pad_nums])
            self.x_test = np.array([self.embedder[pad_sent, ] for pad_sent in self.train_pad_nums])
